...
static void load_data(int tc)
{
    ...
    if (tc <= 4) {
        scanf("%d", &req_size);
        for (register int i = 0; i < req_size; ++i)
            scanf("%d", &req_q[i]);
        scanf("%d", &cmd_size);
        for (register int i = 0; i < cmd_size; ++i)
            scanf("%d", &cmd_q[i]);
        scanf("%d", &answer_size);
        for (register int i = 0; i < answer_size; ++i)
            scanf("%d", &answer[i]);
    } else {
...
static int run(int tc)
{
    ...
    for (register int i = 0; i < track_size; ++i)
        flag[i] = false;
    ...
    while (req_size != req_cnt || (tc <= 4 && cmd_size != cmd_idx)) {
        ...
        if (CMD_FCFS <= cmd && CMD_CLOOK >= cmd && (req_cnt - answer_idx) > 0) {
            ...
            if (tc <= 4)
                new_track = req_q[req_idx++];
            else {
                new_track = mrand(track_size);
                while (flag[new_track]) {
                    ++new_track;
                    if (new_track == track_size)
                        new_track = 0;
                }
            }
