...
extern void add(char mName[MAX_NAME_LEN], int mTypeNum,
                char mTypes[MAX_N][MAX_TAG_LEN], int mSection);
...
extern int countBook(int mTypeNum, char mTypes[MAX_N][MAX_TAG_LEN],
                     int mSection);
...
static bool run()
{
    ...
    for (int q = 0; q <= Q; ++q) {
        ...
        case CMD_MOVETYPE:
            ...
            scanf("%d", &ans);
            if (ret != ans) {
                printf("MOVE %s %d %d should be %d not %d\n", mType, mFrom, mTo,
                       ans, ret);
                okay = false;
            }
            break;
        ...
        case CMD_COUNTBOOK:
            ...
            if (ret != ans) {
                okay = false;
                printf("COUNT %d ... %d should be %d not %d\n", mTypeNum,
                       mSection, ans, ret);
            }
            break;
        }
