...
int createDirectory(char userName[], char path[], char directoryName[],
                    int permission);
...
static int run(int Ans)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (type == 1) {
            ...
            int userAnswer =
                createDirectory(userName, path, directoryName, permission);
            scanf("%d", &expectedAnswer);
            if (userAnswer != expectedAnswer) {
                Ans = 0;
                printf("createDirectory %s %s %s %d should be %d not %d\n",
            ...
            if (userAnswer != expectedAnswer) {
                Ans = 0;
                printf("createFile %s %s %s %s should be %d not %d\n", userName,
                       path, fileName, fileExt, expectedAnswer, userAnswer);
            }
        } else if (type == 4) {
            ...
            if (userAnswer != expectedAnswer) {
                Ans = 0;
                printf("find %s %s should be %d not %d\n", userName, pattern,
                       expectedAnswer, userAnswer);
            }
        }
    }
