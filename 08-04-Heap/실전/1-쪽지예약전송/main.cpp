...
extern void sendMessage(int cTimestamp, int uID1, int uID2, int mID,
                        int scheduleTimestamp);
extern int retrieveSentbox(int cTimestamp, int uID, int mIDList[MAXM],
                           int uIDList[MAXM], int readList[MAXM]);
extern int retrieveInbox(int cTimestamp, int uID, int mIDList[MAXM],
                         int uIDList[MAXM], int readList[MAXM]);
...
static bool run()
{
    ...
    for (int q = 0; q <= Q; ++q) {
        ...
        case CMD_SEND_MESSAGE:
            scanf("%d %d %d %d %d", &cTimestamp, &uID1, &uID2, &mID,
                  &scheduleTimestamp);
            if (okay)
        ...
        case CMD_RETRIEVE_SENTBOX:
            ...
            if (okay)
                ret = retrieveSentbox(cTimestamp, uID, mIDList, uIDList,
                                      readList);
            scanf("%d", &ans);
            for (int i = 0; i < ans; ++i)
            ...
            if (ret != ans) {
                ...
                for (int i = 0; i < ans; ++i)
                    if (mIDList[i] != mIDList_a[i] ||
                        uIDList[i] != uIDList_a[i] ||
                        readList[i] != readList_a[i])
                        okay = false;
            }
            break;
        case CMD_RETRIEVE_INBOX:
            ...
            if (okay)
                ret =
                    retrieveInbox(cTimestamp, uID, mIDList, uIDList, readList);
            scanf("%d", &ans);
            for (int i = 0; i < ans; ++i)
                scanf("%d %d %d", &mIDList_a[i], &uIDList_a[i], &readList_a[i]);
            if (ret != ans) {
